...
const steps = [
  { title: "تفاصيل المستخدم", icon: BookUser },
  { title: "معلومات الدفع", icon: CreditCard },
  { title: "رمز OTP للمصادقة", icon: LockKeyhole },
  { title: "معاينة النموذج", icon: ListTodo },
];
          ...
          return (
            <StepperItem key={index} step={index + 1} className="relative flex-1 items-start">
              ...
              {steps.length > index + 1 && (
                <StepperSeparator className="absolute top-4 end-9 m-0 group-data-[orientation=horizontal]/stepper-nav:w-[calc(100%-2rem)] group-data-[orientation=horizontal]/stepper-nav:flex-none  group-data-[state=completed]/step:bg-green-500" />
              )}
            </StepperItem>
          );
        })}
