  ...
  return (
    <div className="space-y-6" dir="rtl">
          ...
          {[1, 2, 3, 4, 5].map((step) => (
            <StepperContent key={step} value={step}>
              <div className="rounded-lg border bg-card p-6 text-card-foreground">
                <h3 className="font-semibold mb-2">الخطوة {step}</h3>
                <p className="text-sm text-muted-foreground mb-4">
                  {step === 5
                    ? "لقد أكملت جميع الخطوات!"
                    : `أكمل الخطوة ${step} للمتابعة.`}
                </p>
                <div className="flex gap-2">
                  {step < totalSteps && (
                    <Button onClick={handleNext}>
                      {step === totalSteps - 1 ? "إنهاء" : "متابعة"}
                    </Button>
                  )}
                  {step === totalSteps && (
                    <Button onClick={handleReset}>ابدأ من جديد</Button>
                  )}
                </div>
              </div>
            </StepperContent>
      ...
      <div className="text-center text-sm text-muted-foreground">
        التقدم: {Math.round((activeStep / totalSteps) * 100)}%
      </div>
    </div>
  );
}
