...
export default function TreePlusMinus() {
  ...
  return (
    <div className="self-start lg:w-[225px]" dir="rtl">
      <Tree
        className="relative before:absolute before:inset-0 before:-me-1.25 before:bg-[repeating-linear-gradient(to_left,transparent_0,transparent_calc(var(--tree-indent)-1px),var(--border)_calc(var(--tree-indent)-1px),var(--border)_calc(var(--tree-indent)))]"
        indent={indent}
        tree={tree}
          ...
          return (
            <TreeItem key={item.getId()} item={item}>
              <TreeItemLabel className="before:bg-background relative before:absolute before:inset-x-0 before:-inset-y-0.5 before:-z-10">
                <span className="flex items-center gap-2 me-1">
                  {item.isFolder() ? (
                    item.isExpanded() ? (
