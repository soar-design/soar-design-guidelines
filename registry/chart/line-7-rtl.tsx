...
const businessCards = [
  {
    title: "تباين الإيرادات",
    metric: "التغيير الشهري من الأساس",
    baseValue: "+15K",
    baseCurrency: "البداية",
    targetValue: "-15K",
    targetCurrency: "النهاية",
    data: revenueData,
    change: "متقلب",
    isPositive: false,
  ...
  {
    title: "تغيير التحويل",
    metric: "تباين المعدل من الصفر",
    baseValue: "50%",
    baseCurrency: "الأساس",
    targetValue: "+60.5%",
    targetCurrency: "الحالي",
    data: conversionData,
    change: "دوري",
    isPositive: true,
                          ...
                          content={({ active, payload, coordinate }) => {
                            if (active && payload && payload.length && coordinate) {
                              ...
                              const formatValue = (val: number) => {
                                if (card.title === "تباين الإيرادات") {
                                  return val >= 0
                                    ? `+$${val.toLocaleString()}`
                                    : `-$${Math.abs(val).toLocaleString()}`;
                                } else if (card.title === "تغيير التحويل") {
