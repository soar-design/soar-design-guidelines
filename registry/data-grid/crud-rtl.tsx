...
import { CheckCircle2, Ellipsis, Filter, Search, UserRoundPlus, X } from "lucide-react";
...
function ActionsCell({ row }: { row: Row<IData> }) {
  const handleCopyId = async () => {
    try {
      ...
      toast.success(`تم نسخ معرف الموظف بنجاح: ${row.original.id}`);
    } catch (error) {
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent side="bottom" align="end">
        <DropdownMenuItem onClick={() => {}}>تعديل</DropdownMenuItem>
        <DropdownMenuItem onClick={handleCopyId}>نسخ المعرف</DropdownMenuItem>
        <DropdownMenuSeparator />
